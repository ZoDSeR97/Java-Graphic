// twoClipPoly.java: Clipping a polygon.
// Uses: Point2D (Section 1.4).
import java.awt.*;
import java.awt.event.*;
import java.util.*;

public class twoClipPoly extends Frame {
   public static void main(String[] args) {new twoClipPoly();   }
...
class CvClipPoly extends Canvas {
   ...
   public void paint(Graphics g) {
      initgr();
      //rWidth ~~ 1+rHeight
      float x1min = (-rWidth / 4)-3, x1max = (rWidth / 4)-3, 
            y1min = (-rHeight / 3), y1max = (rHeight / 3);
      // Draw clipping square:
      g.setColor(Color.blue);
      drawLine(g, x1min, y1min, x1max, y1min);
      drawLine(g, x1max, y1min, x1max, y1max);
      drawLine(g, x1max, y1max, x1min, y1max);
      drawLine(g, x1min, y1max, x1min, y1min);
      ...
      if (!ready) { // Show tiny rectangle around first vertex:
         ...
         poly.clip(x1min, y1min, x1max, y1max);
         System.out.println(poly);
         System.out.println(poly.v);
         drawPoly(g, poly);
      }
   }
}
...
class Poly {
   ...
   void clip(float xmin, float ymin, float xmax, float ymax) { 
      ...
      for (int i = 0; i < n; i++) {
         ...
         if (aIns != bIns)
            poly1.addVertex(new Point2D(xmax, a.y + (b.y - a.y) * (xmax - a.x) / (b.x - a.x)));
      ...
      for (int i = 0; i < n; i++) {
         ...
         if (aIns != bIns)
            poly1.addVertex(new Point2D(xmin, a.y + (b.y - a.y) * (xmin - a.x) / (b.x - a.x)));
         if (bIns) poly1.addVertex(b);
      }
      v = poly1.v; poly1 = new Poly();
